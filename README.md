...
<B>M_Price_2_3.py</B> - прогнозирование/эксплуатация с запросом API обновлённая версия. Изменения касаются формата вывода результатов. Дальнейшее описание соответсвует этой версии.<BR>
...
<pre>
	{
	"code": 0, # код завершения
  	"csv_name": "results/prognose_2025-08-22_16-08-35.csv",  # имя csv-файла с результатом
  	"excel_name": "results/prognose_2025-08-22_16-08-35.xlsx",  # имя Excel-файла с результатом
	"f_month": 8,  # месац прогноза
	...
	"lat": 37.61191755, # долгота
	"lon": 55.7965029,  # широта
	"official": 1.097,  # индекс по официальным данным в соответствии с Распоряжением № ДПР-Р-34.24 28.12.2024
	"price_index": 1.156, # прогноз индекса цен на заданный горизонт (при подозрении на неадекватность комментарий в 'response')
	"pricemetr25": 442.57, # рассчитанная цена кв. метра в тысячах рублей на конец II квартала 2025
	"pricemetr_dom": 511.47, # цена кв. метра в тысячах рублей на заданный горизонт по индексу Домклик
	"pricemetr_off": 485.71, # цена кв. метра в тысячах рублей на заданный горизонт по Распоряжению № ДПР-Р-34.24 28.12.2024
	"response": "Okay"
	}
 </pre>
Код завершения: 
...
|Широта		|Долгота	|количество  	|общая площадь 	|жилая площадь	|Горизонт	|Индекс 		|Индекс 	|Цена за метр		|Цена за метр	|Цена за метр 	|Примечание	|
|			|			|этажей всего 	|объекта		|объекта		|прогноза	|официальный	|Домклик	|на 2кв. 2025 (тыр)	|по Домклик 	|по официальной	|			|
------------+-----------+---------------+---------------+---------------+-----------+---------------+-----------+-------------------+---------------+---------------+------------
|55.7965029	|37.61191755|56				|38910.0		|22301.0		|2.0		|1.097			|1.156		|442.57				|511.47			|485.71			|Okay		|
------------+-----------+---------------+---------------+---------------+-----------+---------------+-----------+-------------------+---------------+---------------+------------
</pre>
